...
public class ClienteControllerView {
	...
	public String cadastrarGerencia(@ModelAttribute Cliente cliente, BindingResult resultado, Model model) {
		...
		if (resultado.hasErrors()) {
			...
			String resposta = clienteService.adicionarCliente(cliente.getUsuario().getEmail(), cliente.getUsuario().getSenha(), cliente.getNome(), cliente.getCpf(),
					cliente.getTelefone());
			try {
	...
	public String cadastrarCliente(@RequestParam("email") String email, @RequestParam("senha") String senha,
		...
		if (resultado.hasErrors()) {
			...
			String resposta = clienteService.adicionarCliente(email, senha, cliente.getNome(), cliente.getCpf(), cliente.getTelefone());
			try {
	...
	public String atualizarCliente(@PathVariable("id") Long id, @ModelAttribute Cliente cliente, BindingResult resultado, Model model) {
		...
		if (resultado.hasErrors()) {
			...
			String resposta = clienteService.atualizarCliente(id, cliente.getUsuario().getEmail(), senha, cliente.getNome(), cliente.getCpf(), cliente.getTelefone());
			carteiraService.AtualizarSaldo(cliente2.getCarteira().getId(), cliente.getCarteira().getSaldoDisponivel());
